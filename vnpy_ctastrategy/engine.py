...
class CtaEngine(BaseEngine):
    ...
    def init_engine(self) -> None:
        ...
        self.register_event()
        self.write_log(tr("Strategy engine initialized successfully", "CTA策略引擎初始化成功"))
    ...
    def init_datafeed(self) -> None:
        ...
        if result:
            self.write_log(tr("Datafeed initialized successfully", "数据服务初始化成功"))
    ...
    def cancel_server_order(self, strategy: CtaTemplate, vt_orderid: str) -> None:
        ...
        if not order:
            self.write_log(f"{tr('Cancel failed, no order found','撤单失败，找不到委托')}-{vt_orderid}", strategy)
            return
    ...
    def send_order(
        ...
        if not contract:
            self.write_log(f"{tr('Send fail,could not find contract', '委托失败，找不到合约')}：{strategy.vt_symbol}", strategy)
            return ""
    ...
    def add_strategy(
        ...
        if "." not in vt_symbol:
            self.write_log(tr("Failed to create strategy,local symbol is missing exchange suffix", "创建策略失败，本地代码缺失交易所后缀"))
            return

        _, exchange_str = vt_symbol.split(".")
        if exchange_str not in Exchange.__members__:
            self.write_log(tr("Failed to create strategy, incorrect exchange suffix for local symbol", "创建策略失败，本地代码的交易所后缀不正确"))
            return
    ...
    def _init_strategy(self, strategy_name: str) -> None:
        ...
        if contract:
            ...
            self.write_log(f"{tr('Contract subscription failed, no contract found','行情订阅失败，找不到合约')}{strategy.vt_symbol}", strategy)
    ...
    def remove_strategy(self, strategy_name: str) -> bool:
        ...
        self.write_log(f"{tr('Strategy','策略')} {strategy.strategy_name} {tr('Removed successfully','移除成功')}")
        return True
    ...
    def send_email(self, msg: str, strategy: CtaTemplate = None) -> None:
        ...
        if strategy:
            ...
            subject: str = tr("Strategy engin", "CTA策略引擎")
